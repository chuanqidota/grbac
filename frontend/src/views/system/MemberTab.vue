...
async function fetchMembers() {
  loading.value = true
  try {
    const data: any = await getSystemMembers(props.systemId)
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    if (!valid || !form.value.user_id) return

    submitting.value = true
    try {
      await addSystemMember(props.systemId, {
...
async function handleRemove(member: Member) {
  try {
    ...
    await removeSystemMember(props.systemId, member.id)
    ElMessage.success('成员移除成功')
    fetchMembers()
  } catch (error: any) {
    if (error !== 'cancel') {
      ElMessage.error(error.message || '成员移除失败')
    }
  }
}
